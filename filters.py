...
mask = cv2.imread("glass_mask.png")
...
glass = imutils.rotate_bound(glass,angle)


glass = imutils.resize(glass,width=g_width)

glass_height = glass.shape[0]

mask  = cv2.cvtColor(mask,cv2.COLOR_BGR2GRAY)
mask = cv2.threshold(mask,0,255,cv2.THRESH_BINARY)[1]
mask = imutils.rotate_bound(mask,angle)
mask = imutils.resize(mask,width = g_width, inter=cv2.INTER_NEAREST)

output = overlay_image(img, glass, mask,shift(g_width,glass_height,angle,re_center[0],re_center[1]))
